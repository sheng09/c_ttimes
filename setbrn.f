...
c
      save 
      include 'limits.inc'
      logical ic,odadd,odall,ocore,lwarn
...
c     2) If an inner core, the second P phase must be the inner core reflection;
...
c     check P names for K and i.
      do i=1,lcb(1)
	 if (0.eq.index(code(i,1),'K')) exit
      enddo
      if(ic)then
         l=3
	 lwarn=i-1.ne.3 .or. 0.eq.index(code(2,1),'i')
	 m=3
      else
         l=2
	 lwarn=i-1.ne.1
	 m=1
      ...
      lwarn=lwarn.or.0.eq.index(code(i,2),'c')
...
c   P, Pdiff, and PKP.
      ...
      call mkdbr(1,lbb(1),-1,l,1,1,dir)
c   PKiKP.
      print *,'Layout:  do PKiKP'
      if(ic) call mkrbr(2,       -1,2,1,1,dir)
c   pP.
      print *,'Layout:  do pP'
      call mkdbr(1,lbb(1),+1,l,1,1,dir)
c   sP.
      print *,'Layout:  do sP'
      call mkdbr(1,lbb(1),+2,l,1,1,dir)
c   pPKiKP.
      print *,'Layout:  do pPKiKP'
      if(ic) call mkrbr(2,       +1,2,1,1,dir)
c   sPKiKP.
      print *,'Layout:  do sPKiKP'
      if(ic) call mkrbr(2,       +2,2,1,1,dir)
c   PcP.
      print *,'Layout:  do PcP'
      call mkrbr(m,       -1,1,1,1,dir)
c   pPcP.
      if (odadd) then
         print *,'Layout:  do pPcP'
         call mkrbr(m,       +1,1,1,1,dir)
c   sPcP.
         print *,'Layout:  do sPcP'
         call mkrbr(m,       +2,1,1,1,dir)
      endif
c   ScP.
      print *,'Layout:  do ScP'
      call mkrbr(m,       -2,1,2,1,dir)
c   pScP (same as pPcS).
      if (odadd) then
         print *,'Layout:  do pScP/pPcS'
         call mkrbr(m,       +1,1,2,1,dir)
c   sScP (same as sPcS).
         print *,'Layout:  do sScP/sPcS'
         call mkrbr(m,       +2,1,2,1,dir)
      endif
c   SKP.
      print *,'Layout:  do SKP'
      call mkdbr(1,m,     -2,l,2,1,dir)
c   pSKP (same as pPKS).
      if (odadd) then
         print *,'Layout:  do pSKP/pPKS'
         call mkdbr(1,m,     +1,l,2,1,dir)
c   sSKP (same as sPKS).
         print *,'Layout:  do sSKP/sPKS'
         call mkdbr(1,m,     +2,l,2,1,dir)
      endif
c   SKiKP.
      print *,'Layout:  do SKiKP'
      if(ic) call mkrbr(2,       -2,2,2,1,dir)
c   PKKP.
      print *,'Layout:  do PKKP'
      call mkdbr(1,m,     -1,l,1,1,cref)
c   SKKP.
      print *,'Layout:  do SKKP'
      call mkdbr(1,m,     -2,l,2,1,cref)
c   PP and P'P'.
      print *,'Layout:  do PP, P''P'''
      call mkdbr(1,lbb(1),-1,l,1,1,sref)
      if (odadd) then
c   pPP and pP'P'.
         print *,'Layout:  do pPP, pP''P'''
         call mkdbr(1,lbb(1),+1,l,1,1,sref)
c   sPP and sP'P'.
         print *,'Layout:  do sPP, sP''P'''
         call mkdbr(1,lbb(1),+2,l,1,1,sref)
      endif
c   S (up-going branch).
      print *,'Layout:  do Sup'
      call mkubr(ku(2),   +2)
c   S, Sdiff, and SKS.
      ...
      call mkdbr(1,lbb(2),-2,l,2,2,dir)
c   pS
      print *,'Layout:  do pS'
      call mkdbr(1,lbb(1),+1,l,2,2,dir)
c   sS
      print *,'Layout:  do sS'
      call mkdbr(1,lbb(2),+2,l,2,2,dir)
c   ScS
      print *,'Layout:  do ScS'
      call mkrbr(m+1,     -2,1,2,2,dir)
      if (odadd) then
c   pScS
         print *,'Layout:  do pScS'
         call mkrbr(m+1,     +1,1,2,2,dir)
c   sScS
         print *,'Layout:  do sScS'
         call mkrbr(m+1,     +2,1,2,2,dir)
      endif
c   PcS
      print *,'Layout:  do PcS'
      call mkrbr(m,       -1,1,1,2,dir)
c   PKS
      print *,'Layout:  do PKS'
      call mkdbr(1,m,     -1,l,1,2,dir)
c   PKKS
      print *,'Layout:  do PKKS'
      call mkdbr(1,m,     -1,l,1,2,cref)
c   SKKS
      print *,'Layout:  do SKKS'
      call mkdbr(1,m,     -2,l,2,2,cref)
c   S3KS
      if (ocore) then
         print *,'Layout:  do S3KS'
         call mkdbr(1,m,     -2,l,2,2,c3ref)
c   S4KS
         print *,'Layout:  do S4KS'
         call mkdbr(1,m,     -2,l,2,2,c4ref)
c   S5KS
         print *,'Layout:  do S5KS'
         call mkdbr(1,m,     -2,l,2,2,c5ref)
c   P3KP.
         print *,'Layout:  do P3KP'
         call mkdbr(1,m,     -1,l,1,1,c3ref)
c   P4KP.
         print *,'Layout:  do P4KP'
         call mkdbr(1,m,     -1,l,1,1,c4ref)
      endif
c   SS and S'S'.
      print *,'Layout:  do SS and S''S'''
      call mkdbr(1,lbb(2),-2,l,2,2,sref)
c   pSS and pS'S'.
      if (odadd) then
         print *,'Layout:  do pSS and pS''S'''
         call mkdbr(1,lbb(1),+1,l,2,2,sref)
c   sSS and sS'S'.
         print *,'Layout:  do pSS and pS''S'''
         call mkdbr(1,lbb(2),+2,l,2,2,sref)
      endif
c   SP
      print *,'Layout:  do SP'
      call mkcbr(m+1,lbb(1),-2,1,2,1,sref)
c   PS
      print *,'Layout:  do PS'
      call mkcbr(m+1,lbb(1),-1,1,1,2,sref)
      return
      end
c
...
c
      ...
      k=1
      ki=kk(k,nph,1)
      if (ki.le.0) call die('**KSEQ:  Bad table index.')
      do 8 i=1,n1
