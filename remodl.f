...
c
 ...
 7    if(j-j1.le.k-k1) go to 8
      do 9 l=j1,j
      n=n+1
      if (n .gt. nsl1) call die('n>nsl1 - nbr slownesses exceeded')
 9    pb(n)=pp(l,1)
      go to 10
 8    do 11 l=k1,k
      n=n+1
...
c
      ...
      open(nout,file='remodl.tbl',access='direct',recl=ndasr)
      n1=kb(2)
...
cgrh
 ...
 16   xp(i,nph)=0d0
      ...
      pmi=pm(1,nph)
cgrh
      if (mm .gt. ndp1) call die('mm > ndp1')
cgrh
...
c     print *,'int',i,k,pb(k),pmj,pmi,dz,ttau,tx
 ...
 27   lz=lz+1
cgrh
      if (lz .gt. nlvz0) call die('lz>nlvz0')
cgrh
      lvz(lz,nph)=n
...
c
 17   continue
      close(nout)
c
      open(nout,file='remodl.hed',form='unformatted')
...
c     data tol,vtol/1e-6,5e-3/
c
c   Get the radii of model discontinuities.
      call emdld(np,rd,modnam)
      if (np.gt.ndmx) call die('**Too many discontinuities in model.')
c   Save the radii of the inner core-outer core and core-mantle boundaries
c   respectively.
      ric = 0
      roc = 0
      do 10 i=1,np
	 call emdlv(rd(i)-0.5,vpc,vscm)
	 call emdlv(rd(i)+0.5,vpc,vscp)
	 if (vscm .gt. 0.0 .and. vscp .eq. 0.0) ric=rd(i)
	 if (vscm .eq. 0.0 .and. vscp .gt. 0.0) roc=rd(i)
	 if (ric*roc .ne. 0) go to 11
10    continue
      if (roc.eq.0) then
         call die('**Bad model, can''t find core discontinuities.')
      else if (ric.eq.0) then
         write(10,*) 'No inner core in this model.'
      endif
11    continue
c
...
c   Set the outer most point of the layer.
      n=n+1
      if (n.gt.nmd0) call die('**Too many layers in model.')
      r(n)=r0
      call emdlv(r0*(1.-tol),vp(n),vs(n))
c   Check for continuity across an apparent discontinuity.
      if(n.le.1) go to 2
...
c   If vs is close to continuous, force it.
 6    if(abs(vs(n-1)-vs(n)).gt.vtol*vs(n)) go to 2
      ...
      vs(n)=vs(n-1)
c   Make P and S velocity the same if we are in the core.
 2    if(vs(n).le.0.0) vs(n)=vp(n)
...
c   Make P and S velocity the same if we are in the core.
      if(vs(n).le.0.0) vs(n)=vp(n)
c   Set the index to the core-mantle radius (this vs zero, prev. is vp)
      call emdlv(r1+0.5,vpc,vscp)
      call emdlv(r1-0.5,vpc,vscm)
      if (vscp.ne.0.0.and.vscm.le.0.0) ncr=n
      i=i-1
...
c
      ...
      kfl=1
      j=1
      call crtpt(1,1,'first P')
      call crtpt(1,2,'first S')
      do 5 i=2,nk
      if(abs(z(j)-z(i)).gt.tol) go to 6
      call crtpt(j,1,'z disc. P')
      call crtpt(i,1,'z disc. below P')
      if(j.le.ncr) call crtpt(j,2,'z disc. S')
      if(i.le.ncr) call crtpt(i,2,'z disc. below S')
      go to 5
 6    go to (7,8),ifl
 7    if(u(i,1).le.u(j,1)) go to 2
      ifl=2
      call crtpt(j,1,'top low vel./slow. zone P')
      go to 2
 8    if(u(i,1).ge.u(j,1)) go to 2
      ifl=1
      call crtpt(j,1,'bot. low vel./slow. zone P')
 2    if(i.gt.ncr) go to 5
      go to (3,4),kfl
 3    if(u(i,2).le.u(j,2)) go to 5
      kfl=2
      call crtpt(j,2,'top low vel./slow. zone S')
      go to 5
 4    if(u(i,2).ge.u(j,2)) go to 5
      kfl=1
      call crtpt(j,2,'bot. low vel./slow. zone S')
 5    j=i
      ucrt(nc+1)=0.
      return
      end
c
      subroutine crtpt(k,nph,why)
c
...
c
      ...
      data nc/0/,tol/1e-6/
      if (nc.eq.0) then
         write(10,*) 'Critical point rationale:'
      else
c        Eliminate duplicates.
         do i=1,nc
            if (abs(ucrt(i)-u(k,nph)).le.tol) then
               write(10,*)'duplicate critical value eliminated:',
     &            u(k,nph),' (',why,')'
               return
           endif
        enddo
      ...
      nc=nc+1
      ucrt(nc)=u(k,nph)
      write(10,'(i2,1x,f10.6,2h: ,a)') nc,ucrt(nc),why
...
c
 ...
 1    continue
      call die('**PGRID:  Critical point not in U table')
 2    ic=i
c
      n=0
      j=ic+1
...
c
 ...
 12   x0=x0+dx
      n=n+1
cgrh
      if (n .gt. nsl1) call die('n>nsl1')
cgrh
      if(abs(x0-xs(l)).le.tol) then
	 pp(n,nph)=ps(l)
	 xx(n,nph)=xs(l)
	 rr(n,nph)=r1
	 go to 22
...
c   Set up iteration with first two trial points, x1 and x2.
 ...
 8    ps0=p1
      ...
      ps1=p0
      xs1=y(1)
 9    xi=(dble(p0)*yi-dble(p1)*y(1))/(yi-y(1))
c   Iterate.
      do 1 m=2,20
...
c   Otherwise, mark the grazing ray to the top of the zone.
      lb=lb+1
cgrh
      if (lb .gt. nbr2) call die('lb>nbr2')
cgrh
...
c
 ...
 1    lc=lc+1
cgrh
      if (lc .gt. nbr1) call die('lc>nbr1')
cgrh
 ...
 4    continue
      ...
      return
 5    if(abs(r0-ric).gt.20..or.ric.eq.0.) go to 2
      pre='KI'
      suf='K'
      if(kfl.le.0) return
      lc=lc+1
